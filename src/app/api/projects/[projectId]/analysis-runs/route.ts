...
export async function GET(_req: Request, ctx: { params: Promise<{ projectId: string }> }) {
  ...
  const { data, error } = await supabase
    .from("analysis_runs")
    .select("id, status, created_at, error, overpass_remark")
    .eq("project_id", projectId)
    .order("created_at", { ascending: false });

  if (error) {
    logError("analysis_runs_list_failed", { message: error.message });
    return jsonError("Failed to load analysis runs", 500);
  }
